...
class WallFollowerController:
    ...
    def calculate_servo_angle(self):
        ...
        left = self.front_left_distance
        right = self.front_right_distance
        front = self.front_center_distance

        # 左と右を見て、前もオッケーだったら前進
        if left > 25 and right > 25 and front > 60:
            rospy.loginfo("前進")
            return 90
        #　左が壁に近づいたら右折
        elif left < 25 and right > 25 and front > 60:
            rospy.loginfo("右折")
            return 180
        # 右が壁に近づいたら左折
        elif left > 25 and right < 25 and front > 60:
        ...
        # right_front_distance_safe = 90  # 右前方の安全距離
        # front_distance_stop = 5  # 前方の停止距離
